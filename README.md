<img src="assets/images/slac_logo.jpg" alt="SLAC-LCLS Logo" width="200">
...
2. **Create Virtual Environment And Install Dependencies**:
   - **From Environment Build File (Recommended):**
     ```bash
     conda env create --file env.yaml
     conda activate env
     ```
     See `env.yaml` for full list of conda dependencies.
   - **From Scratch:**
     ```bash
     ...
     ```
     Replace all instances of `env_name` with the desired virtual environment name.

     See `requirements.txt` for full list of pip dependencies.
...
3. **Configure Your Google Sheet:**
	...
	- Make sure your sheet includes two separate tabs (worksheets):
		...
		- One named "Users" — this should store usernames for access control, logging, or tracking who checks out
		  stock. All usernames should be placed in the first column.
...
5. **Running The Application:**
	- **Run The GUI:**
	  ...
	  To launch the application in GUI mode, just use the `--run` or `-r` flags:
	   ```bash
