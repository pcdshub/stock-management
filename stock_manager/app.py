"""
Instantiate and run the App class to start
the SLAC Inventory Management application.
"""
...
class App(QMainWindow):
    ...
    Handles UI loading, screen transitions, and application
    ...
    def __init__(self):
        ...
        try:
            ui_path: Path = Path(
                __file__
            ).resolve().parent.parent / 'ui' / 'main.ui'
    ...
    def handle_connections(self) -> None:
        ...
        for button, controller, enum in zip(
                self.buttons,
                self.controllers,
                stock_manager.utils.Pages
        ):
        ...
        def handle_icons():
            ...
            self.log_out_btn.setIcon(
                qta.icon('fa5s.sign-out-alt', color='white')
            )
    ...
    async def _async_load(self) -> None:
        """
        Asynchronously load data from the database
        and update the application tables.

        :raises SystemExit: If the user chooses to
        close the application after a data load failure.
        """
    ...
    def _on_page_changed(self) -> None:
        """
        Update window title and manage QR
        scanner based on current screen.
        """

        if (self.scanner.camera_thread.running
                and self.screens.currentIndex()
                != stock_manager.utils.Pages.SCAN.value.PAGE_INDEX):
            try:
        ...
        def bold_current_screen_button() -> None:
            """
            Update sidebar buttons' font to
            indicate the currently active screen.
            """
    ...
    async def update_tables(self) -> None:
        """
        Refreshes or updates the displayed tables in the UI
        to reflect the latest inventory data.

        This method should be called after making changes to
        the inventory or data source.
        """
