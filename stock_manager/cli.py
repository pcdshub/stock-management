...
def build_commands() -> argparse.Namespace | None:
    ...
    top_parser = argparse.ArgumentParser(prog='stock_manager', description='Common Stock Manager CLI')
    ...
    qr_parser.add_argument(
            ...
            nargs='?',
            default='./exports',
            help='Output Path Where The QR Code Image Will Be Saved (Default Path: ./exports)'
    )
    ...
    item_parser = sub_parsers.add_parser(
            'items',
            help='Commands Related To Item Management (e.g., list, add, search, etc.)'
    )
    ...
    add_parser.add_argument(
            'values',
            nargs=10,
            help='10 Values Representing The Item Fields In Order (e.g., part_num, manufacturer, total, etc.)'
    )
    ...
    edit_parser.add_argument(
            'values',
            nargs='+',
            help='Pairs Of Field Names And New Values To Update (e.g., part_num=sample_item, total=0, etc.)'
    )
    ...
    user_parser = sub_parsers.add_parser(
            'users',
            help='Commands Related To User Management (e.g., list, add, search, etc.)'
    )
...
def _run_export(args) -> bool:
    """
    Handler for the `export...` command --- exports item data to a specified file format and location.
...
def _run_qr(args) -> bool:
    """
    Handler for the `qr...` command --- generates and saves a QR code for an item.
...
def _run_sync_databases(args) -> bool:
    """
    Handler for the `sync` command --- synchronizes the local and remote database.
...
def _run_list_items(args) -> bool:
    """
    Handler for the `...items -l/--list` command --- lists all items in the database.
...
def _run_search_items(args) -> bool:
    """
    Handler for the `...items search...` command --- searches items by a given string.
...
def _run_add_item(args) -> bool:
    """
    Handler for the `...item add...` command --- adds a new item to the database.
    
    :param args: CLI arguments with 10 values representing item detail in the Google Sheet's order.
    :return: `True` if operation is completed successfully, `False` otherwise.
    """
...
def _run_remove_item(args) -> bool:
    """
    Handler for the `...item remove...` command --- removes an item by part number.
...
def _run_edit_item(args) -> bool:
    """
    Handler for the `edit` command --- edits an item by specified part number and values.
    and field specified changes.
    
    :param args: CLI arguments with one `part_num` and up to 9 individual field values.
    :return: `True` if operation is completed successfully, `False` otherwise.
    """
...
def _run_list_users(args) -> bool:
    """
    Handler for the `...users -l/--list` command --- lists all users in the database.
...
def _run_search_users(args) -> bool:
    """
    Handler for the `...users search...` command --- searches users by username substring.
...
def _run_add_user(args) -> bool:
    """
    Handler for the `...users add...` command --- adds a new user to the database.
...
def _run_remove_user(args) -> bool:
    """
    Handler for the `...users remove...` command --- removes a user by username.
    ...
    username_arg = args.username
    if not DBUtils().update_users_database(DatabaseUpdateType.REMOVE, username_arg):
        return False
