...
application. Each abstract class provides required methods, common
functionality, or reusable connection logic for concrete page controllers.
"""
...
class CombinedMeta(type(QWidget), ABCMeta):
    ...
    This enables the AbstractController to inherit from both PyQt widgets
    and Python abstract base classes, resolving metaclass conflicts that
    would otherwise occur with multiple inheritance.
    """
    pass
...
class AbstractController(ABC, QWidget, metaclass=CombinedMeta):
    ...
    def __init__(self, file_name: str, app: 'App'):
        ...
        :param file_name: The name of the .ui file (without extension) to load.
        ...
        try:
            ui_path = Path(
                __file__
            ).resolve().parent.parent.parent / 'ui' / f'{file_name}.ui'
    ...
    async def update_table(self) -> bool:
        ...
        source_model.setHorizontalHeaderLabels(stock_manager.utils.KEEP_HEADERS)
    ...
    def to_page(self) -> None:
        ...
        try:
            self.app.setWindowTitle(
                self.PAGE_NAME.value.PAGE_TITLE
            ...
            self.setWindowTitle(
                'SLAC Inventory Management Application' + username
            )
...
class AbstractScanner(AbstractController):
    ...
    def process_frame(self, frame: ndarray) -> None:
        ...
        Sends the frame to subclass's `check_for_qr()`
        to asynchronously decode and manage QR data.
    ...
    class _CameraThread(QThread):
        ...
        via the `frame_ready` signal for processing elsewhere in
        the application.
        """
...
class AbstractExporter(AbstractController):
    ...
    def get_directory(self, button: QPushButton) -> None:
        ...
        If an exception occurs during selection,
        the user is offered the option to retry.

        :param button: sets the text of `button` to the last 6 characters
        of the user chosen path.
        """
        ...
        try:
            response = QFileDialog.getExistingDirectory(
                ...
                str(
                    Path(
                        __file__
                    ).resolve().parent.parent.parent / 'exports'
                )
            )
