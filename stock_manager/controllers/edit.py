"""
Edit page controller for managing inventory item edits
in the Stock Management Application.

This module provides the Edit class, which handles the UI
and logic for editing inventory items, including updating
item details, validating user input, and saving changes
to the database.
"""
...
class Edit(AbstractController):
    ...
    def _on_cell_clicked(self, index: QModelIndex) -> None:
        """
        Populate the edit form fields with data from the selected
        item when a table row is clicked.
        ...
        try:
            ...
            self.b750_spinner.setValue(
                item.stock_b750
                if item.stock_b750 is not None
                else 0
            )
            self.b757_spinner.setValue(
                item.stock_b757
                if item.stock_b757 is not None
                else 0
            )
            self.min_750_spinner.setValue(
                item.minimum
                if item.minimum is not None
                else 0
            )
            self.min_757_spinner.setValue(
                item.minimum_sallie if item.minimum_sallie is not None
                else 0
            )
    ...
    def _submit_form(self) -> None:
        ...
        if new_item == self._selected_item:
            QMessageBox.information(
                ...
                'Identical Items',
                'Item Is Unchanged, '
                'Please Change A Field Before Submitting Form'
            )
            return
        ...
        self.logger.info(
            f'{self.app.user} Edited Database Item: {new_item.part_num}'
        )
        self.app.update_tables()
        self.database.update_items_database(
            stock_manager.utils.DatabaseUpdateType.EDIT,
            new_item
        )
        self._clear_form()
