"""
Remove page controller for removing inventory data
...
class Remove(AbstractController):
    ...
    def _on_cell_clicked(self, index: QModelIndex) -> None:
        ...
        if response == QMessageBox.Yes:
            ...
            self.logger.info(
                f'{self.app.user} Removed Item From Database: '
                f'{selected_item.part_num}'
            )
            self.app.update_tables()
            self.database.update_items_database(
                stock_manager.utils.DatabaseUpdateType.REMOVE,
                selected_item
            )
