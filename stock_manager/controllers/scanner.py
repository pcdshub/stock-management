...
class ItemScanner(AbstractScanner):
    ...
    def _clear_form(self) -> None:
        """
        Clears all fields in the scanner UI form
        and resets the scanned item list.
        """
    ...
    def _finish_form(self) -> None:
        """
        Handles the completion of a scanning session
        and navigates to the finish screen.
        """
        ...
        try:
                ...
                for item in self.app.all_items:
                    ...
                    if item.stock_b750 + item.stock_b757 <= 0:
                        ...
                        msg = ('Hello,\n\n'
                               ...
                               'detailing that the following item '
                               'has reached a total stock of 0:\n'
                               f'\tItem: {item.part_num}\n'
                               f'\tDescription: {item.description}\n'
                               f'\tExcess Count: {item.excess} ('
                               f'{item.stock_status.value})\n'
            ...
            self.logger.error(
                f'Item(s) Could Not Be Subtracted From Database: {e}'
            )
            self.app.finish.set_text(
                'An Error Occurred, '
                'Item(s) Could Not Be Subtracted From Database.'
            )
        else:
            ...
            self.logger.info(
                f'{self.app.user} Checked Out Items: {string_items}'
            )
            self._clear_form()
        finally:
...
class Login(AbstractScanner):
    ...
    Handles the login functionality for both
    user QR code scan and the login form.
    """
