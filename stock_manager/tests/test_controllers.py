...
from stock_manager.controllers import (AbstractController, AbstractScanner,
                                       Add, Edit, Export, Finish, ItemScanner,
                                       Login, QRGenerate, Remove, View)
...
def handle_alert(
    ...
    select: int = QMessageBox.Ok
):
    monkeypatch.setattr(
        QMessageBox, alert_type, lambda *args, **kwargs: select
    )
    ...
    def test_to_page(self, qtbot: QtBot, controller):
        if isinstance(controller, AbstractScanner):
            skip(
                ...
                'But Lots Of Warnings When Switching Pages'
            )
...
@mark.skip(
    reason='Raises Warnings But No Errors, Clutters All Useful Information'
)
class TestScanners:
...
def test_login(qtbot: QtBot, monkeypatch: MonkeyPatch):
    ...
    handle_alert(monkeypatch, 'warning')
    assert controller.username.text() not in controller._users_list \
           and not controller._login_clicked()

    controller._users_list.append(TEST_USERNAME)
    handle_alert(monkeypatch, 'warning')
    assert (controller.username.text() in controller._users_list
            and controller._login_clicked()
            and controller.app.current_page == controller.app.view.PAGE_NAME
            and controller.app.user == TEST_USERNAME)
